...
## What & Why
Build the HR (Human Resources) module — the staff management system for SchoolOS. Defines staff identity, departments, designations, leave policies, daily staff attendance, and staff document management. This is a Layer 2 Core Domain module. Academics module requires staff for teacher assignments. All staff members are linked to the users/school_memberships system for login. Board-agnostic.

## Done looks like
- Admins can create and manage the staff directory with full profiles (department, designation, join date, qualification, salary grade)
- **Staff HR documents can be uploaded and managed** (offer letter, appointment letter, ID proof, educational certificates, etc.)
...
## Out of scope
- Payroll calculation (Module 32 — Payroll Management — separate module)
- Biometric attendance integration (future)
- Appraisal / performance review (future)
- Recruitment / job posting (future)

## Tasks

1. **DB migration — HR structure tables** — Create migration `008-hr-structure.ts` with:
...
2. **DB migration — leave management** — Create migration `009-hr-leave.ts` with:
...
3. **DB migration — staff attendance** — Create migration `010-hr-attendance.ts` with:
...
4. **DB migration — staff documents** ✅ Created `013-hr-staff-documents.ts` with:
   - `staff_documents`: file metadata per staff (document_type enum: offer_letter/appointment_letter/id_proof/address_proof/educational_certificate/experience_letter/aadhaar_card/pan_card/passport/other, title, file_url, file_name, file_size_kb, mime_type, uploaded_by, notes). Index on `(school_id, staff_id)`.

5. **Departments & Designations endpoints** —
...
7. **Staff profile endpoint** — `PUT /v1/hr/staff/:id/profile` (upsert extended profile). `GET /v1/hr/staff/:id/profile`. Permission: `hr.staff.update`.

8. **Staff document endpoints** ✅ —
   - `GET /v1/hr/staff/:id/documents` — list documents. Permission: `hr.staff.view`.
   - `POST /v1/hr/staff/:id/documents` — add document (file_url from object-storage; store metadata). Audit logged. Permission: `hr.staff.update`.
   - `GET /v1/hr/staff/:id/documents/:docId` — single document. Permission: `hr.staff.view`.
   - `PATCH /v1/hr/staff/:id/documents/:docId` — update title, type, notes. Permission: `hr.staff.update`.
   - `DELETE /v1/hr/staff/:id/documents/:docId` — delete record. Audit logged. Permission: `hr.staff.update`.

9. **Leave types endpoints** — `POST /v1/hr/leave-types`, `GET /v1/hr/leave-types`, `PATCH /v1/hr/leave-types/:id`, `DELETE /v1/hr/leave-types/:id`. Permission: `hr.leave.manage_types`.

10. **Leave allocations endpoints** — `POST /v1/hr/leave-allocations/bulk` (bulk-allocate all leave types to all active staff for an academic year; async BullMQ job; 202 response with job_id), `GET /v1/hr/staff/:id/leave-allocations` (list allocations per staff per year), `PATCH /v1/hr/leave-allocations/:id` (manually adjust allocated days). Permission: `hr.leave.manage_allocations`.

11. **Leave request endpoints** —
...
13. **HR NestJS module** ✅ — `HRModule` wiring all controllers, services, repositories. Export `StaffService` (needed by Academics for teacher assignment). Register in `AppModule`. Includes `StaffDocumentsController` and `StaffDocumentsService`.

14. **Frontend — Staff directory page** (`/dashboard/hr/staff`) — Data table:
...
15. **Frontend — Staff detail page** (`/dashboard/hr/staff/:id`) — Tabbed layout:
    ...
    - **Documents tab**: Document list with type badge, file name, upload date, download/delete actions. "Upload Document" button opens upload form with file picker + type selection (offer_letter, id_proof, etc.).
    - Breadcrumb. Skeleton loaders per tab.

16. **Frontend — Add/Edit Staff form** (slide-over) — Sections:
...
17. **Frontend — Leave management page** (`/dashboard/hr/leave`) — Two tabs:
...
18. **Frontend — Staff attendance page** (`/dashboard/hr/attendance`) — Date picker at top (defaults today). Below: staff list with attendance status selector per row (Present/Absent/Half-Day/On Leave dropdown). "Save All" button at bottom with loading state. Summary stats bar (Present X, Absent Y, etc.). Skeleton loader on date change.

19. **Frontend — HR settings page** (`/dashboard/hr/settings`) — Three sections: Departments, Designations, Leave Types. Each as a card with list + add/edit/delete. Inline edit on click. Toast on mutations. Skeleton loaders.

20. **Frontend — HR navigation** — Add "HR" section to sidebar with sub-items: "Staff", "Leave Management", "Attendance", "Settings". Permission-guard all routes.

21. **Seed HR data** — Update `seed.ts` to create 2 departments (Academic, Administration), 3 designations (Principal, Teacher, Clerk), 3 leave types (CL: 12 days, SL: 10 days, EL: 15 days), and 2 staff members (linked to users). Allocate leave for 2025-26 academic year.
...
- `backend/src/database/migrations/008-hr-structure.ts`
- `backend/src/database/migrations/009-hr-leave.ts`
- `backend/src/database/migrations/010-hr-attendance.ts`
- `backend/src/database/migrations/013-hr-staff-documents.ts`
- `backend/src/database/seeds/seed.ts`
- `backend/src/config/permissions.ts`
- `backend/src/modules/hr/entities/staff-document.entity.ts`
- `backend/src/modules/hr/endpoints/staff-documents/`
